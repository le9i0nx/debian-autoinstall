...
label auto
    kernel /linux
    append auto=true url=https://le9i0nx.github.io/debian-autoinstall/aws.cfg debian-installer/allow_unauthenticated_ssl=true netcfg/choose_interface=auto netcfg/dhcp_timeout=60 keyboard-configuration/xkb-keymap=en priority=critical vga=788 initrd=/initrd.gz --- quiet
    # debian-installer/allow_unauthenticated_ssl=true
EOF
cd ..
genisoimage -r -V "debian auto install" -cache-inodes -J -l -b isolinux.bin  -c boot.cat -no-emul-boot -boot-load-size 4 -boot-info-table -o debian-auto-install-aws.iso iso/
rm -r /tmp/iso
rm -r /tmp/debian
