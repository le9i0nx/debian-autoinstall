...
cat initrd.gz firmware.cpio.gz > /boot/initrd.gz
rm -r /tmp/debian
cd /boot
...
menuentry 'auto install debian' {
    linux    /boot/linux auto=true url=http://le9i0nx.github.io/debian-autoinstall/aws.cfg interface=auto netcfg/dhcp_timeout=60 keyboard-configuration/xkb-keymap=en priority=critical vga=788 --- quiet
